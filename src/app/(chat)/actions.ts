...
import { normalizePublicId, normalizeTagsCSV } from '@/lib/action-helpers';
...
function buildHelpText(): string {
    return [
        'Hi there! I’m here to help you manage your Cloudinary assets. Here’s what works right now:',
        '',
        '• Upload — drag & drop a file or use the paperclip.',
        '• List images — type: list images.',
        '• List images in a folder — e.g. list images in marketing/banners.',
        '• List folders — e.g. list folders or list folders in marketing.',
        '• Rename — e.g. rename old/path/hero to marketing/hero-v2, or rename the above image to hero-v2.',
        '• Move — e.g. move old/path/hero to marketing, or move the above image to marketing.',
        '• Delete — e.g. delete marketing/hero-v2, or delete the above image.',
        '• Tag — e.g. tag marketing/hero-v2 with homepage, hero, or tag the above image with homepage.',
        '• Create folder — e.g. create folder marketing/banners.',
        '',
        'Tip: start with “list images” to see what’s in your account.',
    ].join('\n');
}

function buildUnsupportedText(featureLabel: string): string {
    return [
        `Heads up: “${featureLabel}” isn’t available yet — it’s on the way.`,
        '',
        'Here’s what I can help you with today:',
        '• Upload files',
        '• List images (optionally within a folder)',
        '• List folders (optionally under a base path)',
        '• Rename or move assets',
        '• Delete assets',
        '• Tag assets',
        '• Create folders',
        '',
        'Want to try something now? For example:',
        '• list images',
...
export async function sendMessageAction(
    ...
    try {
        ...
        /* --------------------- Not-yet-implemented detectors ------------------ */
        const unsupportedMap: { re: RegExp; label: string }[] = [
            { re: /\b(list|show)\s+videos?\b/i, label: 'List videos' },
            { re: /\b(list|show)\s+files\b/i, label: 'List files' },
            { re: /\bget[-\s]?asset[-\s]?details?\b/i, label: 'Get asset details' },
            { re: /\bsearch[-\s]?assets?\b/i, label: 'Search assets' },
            { re: /\bvisual[-\s]?search\b/i, label: 'Visual search' },
            { re: /\bdelete[-\s]?folder\b/i, label: 'Delete folder' },
            { re: /\bmove[-\s]?folder\b/i, label: 'Move folder' },
            { re: /\bcreate[-\s]?asset[-\s]?relations?\b/i, label: 'Asset relations' },
            { re: /\bdownload[-\s]?asset[-\s]?backup\b/i, label: 'Download asset backup' },
        ];
        const unsupported = unsupportedMap.find((u) => u.re.test(text));
        ...
        if (listImagesInMatch) {
            ...
            assistantMsg = renamed
                ? {
                    id: crypto.randomUUID(),
                    role: 'assistant',
                    text: `Successfully renamed the asset to “${to_public_id}”.`,
                    assets: [renamed],
                }
                : {
                    id: crypto.randomUUID(),
                    role: 'assistant',
                    text: `I couldn’t rename that asset. The ID “${from_public_id}” may no longer be valid.`,
                };
            ...
            assistantMsg = ok
                ...
                : { id: crypto.randomUUID(), role: 'assistant', text: `I couldn’t delete “${public_id}”. It may not exist.` };
            ...
            assistantMsg = ok
                ? {
                    id: crypto.randomUUID(),
                    role: 'assistant',
                    text: `Added tags to ${public_id}: ${tagsCsv}`,
                    assets: asset ? [asset] : undefined,
                }
                : { id: crypto.randomUUID(), role: 'assistant', text: `I couldn’t tag “${public_id}”.` };
            ...
            assistantMsg = renamed
                ...
                : {
                    id: crypto.randomUUID(),
                    role: 'assistant',
                    text: `I couldn’t rename that asset. Please make sure “${from_public_id}” exists.`,
                };
            ...
            assistantMsg = ok
                ? { id: crypto.randomUUID(), role: 'assistant', text: `Deleted ${public_id}.` }
                : { id: crypto.randomUUID(), role: 'assistant', text: `I couldn’t delete “${public_id}”. Please check the ID.` };
            ...
            assistantMsg = ok
                ...
                : { id: crypto.randomUUID(), role: 'assistant', text: `I couldn’t create “${folderPath}”.` };
            ...
            assistantMsg = moved
                ...
                : { id: crypto.randomUUID(), role: 'assistant', text: 'I couldn’t move that asset.' };
            ...
            // Friendly "not yet implemented" response
            assistantMsg = {
                id: crypto.randomUUID(),
                role: 'assistant',
                text: buildUnsupportedText(unsupported.label),
            };

        } else {
            // Friendly guide (no markdown bullets, no raw tool dump)
            assistantMsg = {
                id: crypto.randomUUID(),
                role: 'assistant',
                text: buildHelpText(),
