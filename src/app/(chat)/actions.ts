...
// ✨ OPTIONAL: conversational polish (only affects text, never tools)
...
export async function sendMessageAction(
    ...
    try {
        ...
        if (file instanceof File) {
            ...
            if (listImagesInMatch) {
                ...
                const filtered = all.filter(
                    (a) =>
                        a.folder === folder ||
                        a.id.replace(/\.[^/.]+$/i, '').startsWith(`${folder}/`),
                );
                assistantMsg = {
                    id: crypto.randomUUID(),
                    role: 'assistant',
                ...
                intent = 'list-folders';
                const base =
                    (listFoldersInMatch ? listFoldersInMatch[3] : '')
                        .trim()
                        .replace(/^\/+|\/+$/g, '') || undefined;
                ...
                assistantMsg = ok
                    ...
                    : {
                        id: crypto.randomUUID(),
                        role: 'assistant',
                        text: `Failed to delete "${public_id}". It may not exist.`,
                    };
                ...
                assistantMsg = ok
                    ...
                    : {
                        id: crypto.randomUUID(),
                        role: 'assistant',
                        text: `Failed to delete "${public_id}". Please check the ID.`,
                    };
                ...
                assistantMsg = ok
                    ...
                    : {
                        id: crypto.randomUUID(),
                        role: 'assistant',
                        text: `Failed to create folder "${folderPath}".`,
                    };
                ...
                // Help with tool list + hint (existing behavior preserved)
                intent = 'help';
        ...
        // ✨ Conversational polish (text only). No tool behavior changes.
        if (assistantMsg) {
            const friendly = await generateFriendlyReply({
                userText: text || (file instanceof File ? `upload ${file.name}` : ''),
                defaultText: assistantMsg.text,
                intent,
                assetsCount: assistantMsg.assets?.length,
                // sprinkle gentle tips mostly for help replies
                extraTips:
                    intent === 'help'
                        ? ['list images', 'list folders', 'rename', 'move', 'delete', 'tag', 'create folder']
                        : undefined,
            });
            assistantMsg.text = friendly;
        }
